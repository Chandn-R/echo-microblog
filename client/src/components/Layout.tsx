...
import { Home, User, Bell, Search, PlusCircle } from "lucide-react";
    ...
    <div className="flex w-full">
      <aside className="w-30 py-4 bg-background flex flex-col items-center h-screen sticky top-0">
        ...
        <nav className="flex flex-col items-center space-y-6 ml-1 flex-1 p-1">
          <button onClick={()=> navigate("/")} className="rounded-lg h-12 w-12 ml-5 brightness-75 hover:brightness-125">
        ...
        <div className="sticky top-0 h-screen p-4 space-y-6 overflow-y-auto">
          <Card>
            <CardHeader className="p-4">
              <CardTitle className="text-base">Trending Threads</CardTitle>
            </CardHeader>
            <CardContent className="p-4 space-y-3">
              {["#React", "#Shadcn", "#ThreadsClone", "#NextJS"].map((tag) => (
                <div
                  key={tag}
                ...
                <div key={user} className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Avatar className="w-8 h-8">
                      <AvatarImage src={`/avatars/suggested-${user}.png`} />
                      <AvatarFallback>SU{user}</AvatarFallback>
                    </Avatar>
                    <span className="text-sm">user_{user}</span>
                  </div>
                  <Button
                    variant="outline"
                    size="sm"
