    ...
    try {
      ...
      if (response.data.success) {
        setUserData({
          ...
          followers: response.data.data?.followers ?? userData.followers,
        });
          ...
          <CardContent>
            {userData.followers.length > 0 ? (
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 gap-4">
                {userData.followers.slice(0, 2).map((follower) => (
                  <div
                    key={follower._id}
        ...
        <Card className="flex-1">
          ...
          <CardContent>
            {userData.following.length > 0 ? (
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 gap-4">
                {userData.following.slice(0, 2).map((following) => (
                  <div
                    key={following._id}
                    className="flex flex-col items-center gap-1 cursor-pointer hover:bg-muted/50 py-2 rounded-lg w-30"
                    onClick={() => navigate(`/users/${following._id}`)}
                  >
                    <Avatar className="h-18 w-18">
                      <AvatarImage
                        src={following.profilePicture?.secure_url}
